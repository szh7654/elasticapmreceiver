...
func GetStartAndEndTimestamps(timestamp uint64, duration uint64) (*pcommon.Timestamp, *pcommon.Timestamp) {
	s := int64(timestamp / 1e9)
	n := int64(timestamp % 1e9)
	startTime := time.Unix(s, n)
	et := timestamp + duration
	s = int64(et / 1e9)
	n = int64(et % 1e9)
	endTime := time.Unix(s, n)
...
func ConvertNanoTime(timestamp uint64) time.Time {
	s := int64(timestamp / 1e9)
	n := int64(timestamp % 1e9)
